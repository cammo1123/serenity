...
class Serenity:
    ...
    def build_target(self, target: str = None):
        if self.target == "lagom":
            ...
            os.system(f'{self.CMAKE_PATH} -S "{self.SERENITY_SOURCE_DIR}/Meta/Lagom" -B "{self.BUILD_DIR}" -DBUILD_LAGOM=ON {extra_cmake_args}')
    ...
    def create_build_dir(self):
        if self.target != "lagom":
            os.system(f'{self.CMAKE_PATH} -GNinja {" ".join(self.CMAKE_ARGS)} -S "{self.SERENITY_SOURCE_DIR}/Meta/CMake/Superbuild" -B "{self.SUPER_BUILD_DIR}"')
        else:
            os.system(f'{self.CMAKE_PATH} -GNinja {" ".join(self.CMAKE_ARGS)} -S "{self.SERENITY_SOURCE_DIR}/Meta/Lagom" -B "{self.SUPER_BUILD_DIR}"')
        

    def ensure_toolchain(self):
        if int(subprocess.getoutput(f'{self.CMAKE_PATH} -P "{self.SERENITY_SOURCE_DIR}/Meta/CMake/cmake-version.cmake"').strip()) != 1:
            self.build_cmake()
    ...
    def build_cmake(self):
        if os.name == "nt":
            print("CMake version check failed, but we don't know how to build CMake on Windows yet. Please install CMake manually.")
            sys.exit(1)
        else:
            print("CMake version check failed, building CMake...")
            os.system(f'cd "{self.SERENITY_SOURCE_DIR}/Toolchain" && ./BuildCMake.sh')
        
    def build_toolchain(self):
        print(f"Building toolchain for {self.target} in {self.TOOLCHAIN_DIR}...")
        if self.toolchain == "Clang":
            os.system(f'cd "{self.SERENITY_SOURCE_DIR}/Toolchain" && ./BuildClang.sh')
        else:
            os.environ["ARCH"] = self.target
            os.system(f'cd "{self.SERENITY_SOURCE_DIR}/Toolchain" && ./BuildIt.sh')
    ...
    def cmd_with_target(self):
        ...
        cmake_path = f"{self.SERENITY_SOURCE_DIR}/Toolchain/Local/cmake/bin"
