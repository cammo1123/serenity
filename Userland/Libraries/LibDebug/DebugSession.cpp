    ...
    Function<void(float)> on_initialization_progress)
{
    ...
    if (auto result = debug_session->update_loaded_libs(); result.is_error()) {
        dbgln("failed to update loaded libs: {}", result.error());
        return {};
    }
...
{
    auto file = TRY(Core::File::open(TRY(String::formatted("/proc/{}/vm", m_debuggee_pid)), Core::File::OpenMode::Read));
