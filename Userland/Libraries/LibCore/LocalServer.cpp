...
{
#if defined(AK_OS_WINDOWS)
    dbgln("Core::LocalServer({}): Socket takeover is not supported on Windows", socket_path);
    VERIFY_NOT_REACHED();
#else
    if (m_listening)
...
{
#if defined(AK_OS_WINDOWS)
    dbgln("Core::LocalServer({}): Socket takeover is not supported on Windows", address);
    VERIFY_NOT_REACHED();
#else
    if (m_listening)
        return false;
