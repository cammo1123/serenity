...
ErrorOr<void> PosixSocketHelper::set_close_on_exec(bool enabled)
{
    ...
    /*auto *handle = reinterpret_cast<HANDLE>(_get_osfhandle(m_fd));
    if (handle == INVALID_HANDLE_VALUE) {
        return Error::from_string_view("Core::Socket::set_close_on_exec: _get_osfhandle failed"sv);
    }

    DWORD handle_info;
    auto res = GetHandleInformation(handle, &handle_info);
    if (res == 0) {
        return Error::from_string_view("Core::Socket::set_close_on_exec: GetHandleInformation failed"sv);
    }

    if (enabled)
        handle_info |= HANDLE_FLAG_INHERIT;
    else
        handle_info &= ~HANDLE_FLAG_INHERIT;

    res = SetHandleInformation(handle, HANDLE_FLAG_INHERIT, handle_info);
    if (res == 0) {
        return Error::from_string_view("Core::Socket::set_close_on_exec: SetHandleInformation failed"sv);
    }*/
...
ErrorOr<NonnullOwnPtr<TCPSocket>> TCPSocket::adopt_fd(int fd)
{
    ...
    auto socket = TRY(adopt_nonnull_own_or_enomem(new (nothrow) TCPSocket()));
    dbgln("Core::TCPSocket: Adopted fd {}", fd);
    socket->m_helper.set_fd(fd);
    socket->setup_notifier();
    return socket;
}
...
ErrorOr<int> LocalSocket::receive_fd(int flags)
{
    ...
    if (WSARecv(m_helper.fd(), &buf, 1, &bytes_received, nullptr, nullptr, nullptr) != 0)
        return Error::from_windows_error(WSAGetLastError());

    if (bytes_received != 1)
        return Error::from_string_literal("Malformed message when receiving file descriptor");

    int fd = c;
    dbgln("LocalSocket::receive_fd: Received fd {}", fd);
...
ErrorOr<void> LocalSocket::send_fd(int fd)
{
    ...
    *((int*)CMSG_DATA(cmsg)) = fd;
    ...
    return {};
#else
    auto message = TRY(String::formatted("F{}", fd)).to_deprecated_string();
    dbgln("LocalSocket::send_fd: Sending fd {}", fd);
    unsigned int length = message.length();
    WSABUF buf {
        .len = length,
        .buf = const_cast<char*>(message.characters())
    };
    DWORD bytes_sent;

    if (WSASend(m_helper.fd(), &buf, 1, &bytes_sent, 0, nullptr, nullptr) != 0)
        return Error::from_windows_error(WSAGetLastError());

    if (bytes_sent != 1)
        return Error::from_string_literal("Failed to send file descriptor");
...
#else
    DWORD nread;
    auto *handle = (HANDLE)_get_osfhandle(m_helper.fd());
